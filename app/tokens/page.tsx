...
export default function TokensPage() {
  ...
  const fetchTokens = async () => {
    ...
    try {
      ...
      // Basit query - where clause olmadan dene
      const tokensCollection = collection(db, "apiTokens")
      const allTokensSnapshot = await getDocs(tokensCollection)

      console.log("All tokens count:", allTokensSnapshot.size)

      const tokensData: APIToken[] = []
      allTokensSnapshot.forEach((doc) => {
        const data = doc.data()
        console.log("Token data:", { id: doc.id, userId: data.userId, currentUser: user.uid })
      ...
      console.log("User tokens:", tokensData.length)
